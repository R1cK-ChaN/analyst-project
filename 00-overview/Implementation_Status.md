...
**Status date:** March 20, 2026 (updated after companion social awareness, stage-as-interaction-freedom, and self-contained web search)
...
The companion agent is now fully self-contained with 3 tools (image gen, live photo, web search) — no external research-service dependency.

`analyst-project` talks to `macro-data-service` over HTTP when `ANALYST_MACRO_DATA_BASE_URL` is set.
...
### Companion agent (self-contained)

Implemented and completed:

- companion is self-contained with 3 tools: `generate_image`, `generate_live_photo` (optional), `web_search`
- `web_search` uses OpenRouter's `:online` model via plugins API — model decides when to search
- single-pass LLM execution with full tool access — no candidate selection or rule-based tool gating
- social awareness: user disengagement detection, self-focus drift detection, question taxonomy, reciprocity enforcement
- stage-as-interaction-freedom: `RelationshipStagePolicy` with per-stage constraints (teasing, disclosure, comfort, disagreement), per-stage example prompt modules, tendency modifiers, engagement ceiling mechanism
- user timezone: extracted from profile, injected into time label alongside bot's Singapore time
- text quality: sentence completeness scoring (formal connectors, compound clauses, comma density), tool artifact sanitization
- `src/analyst/research/` contains unused delegate code preserved for reference; companion does NOT use it
...
- Telegram polling bot with persona-driven agent execution (陈襄)
  ...
  - companion agent exposes 3 tools: image generation + optional live-photo generation + web search (via OpenRouter `:online` model)
...
- Telegram bot deployed to Contabo VPS (`rick@vl`) as systemd service (`analyst-telegram.service`)
- deployment workflow: `git pull && sudo systemctl restart analyst-telegram.service`
- `.env` on server holds production tokens

Not yet implemented:

- config/env management beyond `.env` files (systemd service is implemented)
...
- companion tools: `generate_image`, optional `generate_live_photo`, `web_search` (OpenRouter `:online` model)
