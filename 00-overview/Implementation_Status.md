...
- `ClientProfileUpdate` data model (`profile.py`): 14 dimensions — language, risk profile, watchlist, expertise level, mood, investment horizon, preferred products, AUM range, firm, role, communication style, key concerns, follow-up items, interaction notes
- context builders (`service.py`):
  ...
  - `build_sales_context()` — client interactions + profile for sales agent personalization
...
- Telegram polling bot with persona-driven agent loop (陈襄)
  ...
  - 14 client profile dimensions extracted and accumulated across conversations
- sales chat agent (`sales_chat.py`): standalone agent loop with tool wiring, client profile management, conversation history, and `build_sales_tools()` factory
